...
import pytest
from models.incoming_transaction import IncomingTransaction
from tests.functional.fixtures.auth import access_token_user, auth_header


class TestOutgoingTransactions:
    def setup_method(self):
        self.endpoint = "/api/v1/transactions/incoming/"

    @pytest.mark.asyncio
    async def test_get_user_transactions(
        self, access_token_admin, client, create_incoming_transactions
    ):
        """Проверяем транзакции пользователя"""
        endpoint = "/api/v1/transactions/incoming/users/"
        user_id = str(create_incoming_transactions[0].user_id)
    ...
    async def test_get_transaction(
        ...
        transaction_id = str(create_incoming_transactions[0].id)
    ...
    async def test_create_transaction(
        ...
        transaction = IncomingTransaction(
            amount=randint(1, 10),
            description="",
            category_id=choice(create_incoming_categories).id,
            wallet_id=wallet.id,
    ...
    async def test_delete_transaction(
        ...
        """Удаляем транзакцию"""
        transaction_id_1 = str(create_incoming_transactions[0].id)
        transaction_id_2 = str(create_incoming_transactions[1].id)
        transaction_id_3 = str(create_incoming_transactions[2].id)
    ...
    async def test_update_transaction(
        ...
        transaction = create_incoming_transactions[0]
        ...
        transaction = create_incoming_transactions[1]
        transaction_id = str(transaction.id)
        transaction.amount += 100
        user_id = str(transaction.user_id)
